...
**Generator ordering** (future work, not part of this spec's scope): `generate_resources.py` must run first since it defines the yield constants weapon/module/ship cost formulas reference; `generate_weapons.py` and `generate_modules.py` would each gain a `buildCost` computation step reading those constants; `generate_ships.py` already runs last and would sum hull + fitted-item costs after both.
